...
Change::Change(int w, int h, int m) {
  ...
  textField->setText("Enter the characteristics of the new window");
  ...
  QLabel *label2 = new QLabel(tr("Height:"));
  height = new QSpinBox(this);
  height->setSingleStep(1);
  height->setMinimum(1);
  height->setValue(h);

  QLabel *label3 = new QLabel(tr("Mines:"));
...
void Change::createNew() {
  if (mines->value() >= width->value() * height->value()) {
    ...
    textField->setText("Too many mines!");
