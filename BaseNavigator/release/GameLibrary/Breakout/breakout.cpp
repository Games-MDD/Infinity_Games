...
Breakout::Breakout(QWidget *parent)
    : QWidget(parent), BOTTOM_EDGE(parent->height() * BOTTOM_EDGE_PR / 100) {
  ...
  paddle = new Paddle(this->width(), this->height());

  int k = 0;

    N_OF_BRICKS=(this->height()/20)*(this->width()/41);

    qDebug() << this->geometry();
    bricks.resize(N_OF_BRICKS);
  for (int i=0; i<this->height()/20; i++) {
      for (int j=0; j<this->width()/41; j++) {
